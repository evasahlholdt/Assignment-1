...
token_train$VISIT<-str_replace_all(token_train$VISIT, c("visit1" = "1", "Visit1"= "1","visit2"="2","Visit2"="2","visit3" = "3", "Visit3"= "3","visit4"="4","Visit4"="4","visit5" = "5", "Visit5"= "5","visit6"="6","Visit6"="6"))

#Replacing all visits with numbers, LU_train
LU_train$VISIT<-str_replace_all(LU_train$VISIT, c("visit1" = "1", "Visit1"= "1","visit2"="2","Visit2"="2","visit3" = "3", "Visit3"= "3","visit4"="4","Visit4"="4","visit5" = "5", "Visit5"= "5","visit6"="6","Visit6"="6"))
...
token_train$SUBJ<-str_remove_all(token_train$SUBJ,dot)
token_train$VISIT<-str_remove_all(token_train$VISIT,dot)

#Removing all dots in LU_train:
LU_train$SUBJ<-str_remove_all(LU_train$SUBJ,dot)
LU_train$VISIT<-str_remove_all(LU_train$VISIT,dot)

#Removing all dots in demo_train:
demo_train$SUBJ<-str_remove_all(demo_train$SUBJ,dot)
...
colnames(demo_train)[33]<-"ELR"
colnames(demo_train)[24]<-"MR"

#Selecting relevant columns 
demo_train<-select(demo_train,SUBJ,VISIT,Diagnosis,Ethnicity,Gender,Age,ADOS,MR,ELR,Socialization)
...
tokenLU<-merge(token_train,LU_train,by=c("SUBJ","VISIT"),all=TRUE)

#Merging the "double" df with the last: 
total<-merge(tokenLU,demo_train,by=c("SUBJ","VISIT"),all=TRUE)
...
visit1only<-total%>%select(SUBJ,VISIT,ADOS,MR,ELR,Socialization) %>% filter(VISIT=="1")
...
visit1only<-select(visit1only,-c(VISIT))

View(visit1only)

#Merge visit1only with total
total<-merge(total,visit1only,by=c("SUBJ"),all=TRUE)
...
total_visit1$SUBJ <- as.factor(total_visit1$SUBJ)

total_visit1$SUBJ<-factor(as.numeric(total_visit1$SUBJ))

View(total_visit1)

#Changing gender
total_visit1$Gender<-str_replace_all(total_visit1$Gender, c("1" = "M", "2"= "F"))

#Changing diagnosis
total_visit1$Diagnosis<-str_replace_all(total_visit1$Diagnosis, c("A" = "ASD", "B"= "TD"))

View(total_visit1)

write.csv(total_visit1,file="CleanData")
```
