...
```{r}
#Changing participant name and visit in demo_train df
...
#Replacing all visits with numbers, token_train
visit1<-str_replace_all(token_train$VISIT, c("visit1" = "1", "Visit1"= "1","visit2"="2","Visit2"="2","visit3" = "3", "Visit3"= "3","visit4"="4","Visit4"="4","visit5" = "5", "Visit5"= "5","visit6"="6","Visit6"="6"))

#Mutating to apply new names
token_train_new<-mutate(token_train,VISIT=visit1)

#Replacing all visits with numbers, LU_train
visit1LU<-str_replace_all(LU_train$VISIT, c("visit1" = "1", "Visit1"= "1","visit2"="2","Visit2"="2","visit3" = "3", "Visit3"= "3","visit4"="4","Visit4"="4","visit5" = "5", "Visit5"= "5","visit6"="6","Visit6"="6"))

#Mutating to apply new names
LU_train_new<-mutate(LU_train,VISIT=visit1LU)
...
#Naming . as "dot" to specify (. has a function in the stringr package)
dot <- "\\."

#Removing all dots in token_train:
tt1 <-str_remove_all(token_train_new$SUBJ,dot)
tt2 <-str_remove_all(token_train_new$VISIT,dot)

#Mutating to apply
token_train_1<-mutate(token_train,SUBJ=tt1)
token_train_done<-mutate(token_train_1,VISIT=tt2)
View(token_train_done)

#Removing all dots in LU_train:
lt1 <-str_remove_all(LU_train_new$SUBJ,dot)
lt2 <-str_remove_all(LU_train_new$VISIT,dot)

#Mutating to apply
LU_train_1<-mutate(LU_train,SUBJ=lt1)
LU_train_done<-mutate(LU_train_1,VISIT=lt2)
View(LU_train_done)

#Removing all dots in demo_train:
dt1 <-str_remove_all(demo_train$SUBJ,dot)

#Mutating to apply
demo_train_done<-mutate(demo_train,SUBJ=dt1)
View(demo_train_done)
...
#demo_train
#Renaming columns names for comfort
colnames(demo_train_done)[33]<-"ELR"
colnames(demo_train_done)[24]<-"MR"
View(demo_train_done)

#Selecting relevant columns 
demo_train_select<-select(demo_train_done,SUBJ,VISIT,Diagnosis,Ethnicity,Gender,Age,ADOS,MR,ELR,Socialization)
View(demo_train_select)

#LU_train
#Selecting relevant columns 
LU_train_select<-select(LU_train_done,SUBJ,VISIT,MOT_MLU,CHI_MLU)
View(LU_train_select)

#token_train
#Selecting relevant columns 
token_train_select<-select(token_train_done,SUBJ,VISIT,types_MOT,types_CHI,tokens_MOT,tokens_CHI)
View(token_train_select)
...
#As merge() takes only two dfs I create one consisting of two:
tokenLU<-merge(token_train_select,LU_train_select,by=c("SUBJ","VISIT"),all=TRUE)

#Merging the "double" df with the last: 
total<-merge(tokenLU,demo_train_select,by=c("SUBJ","VISIT"),all=TRUE)
...
#Select relevant variables in new df
selected<-select(total,SUBJ,VISIT,ADOS,MR,ELR,Socialization)

#Filter for only visit 1
visit1only<-filter(selected,VISIT==1)

#Rename variables so that it is clear that it is just for visit 1
colnames(visit1only)[3]<-"ADOS1"
colnames(visit1only)[4]<-"MR1"
colnames(visit1only)[5]<-"ELR1"
colnames(visit1only)[6]<-"Socialization1"

#Remove VISIT variable: 
visit1onlydone<-select(visit1only,-c(VISIT))

#Merge visit1onlydone with total
total<-merge(total,visit1onlydone,by=c("SUBJ"),all=TRUE)

View(total)

#Note: The total df contains BOTH the different data from all visits AND the visit1 for all visits (good to have!)

#Df WITHOUT different data: 

total_visit1<-select(total,-c(ADOS,MR,ELR,Socialization))
...
#Anonymizing the subjects using numbers (NOTE: I have not excluded NAs)

total2<-factor(as.numeric(total_visit1$SUBJ))

#Mutating to apply new names
total3<-mutate(total_visit1,SUBJ=total2)

#Changing gender
total4<-str_replace_all(total3$Gender, c("1" = "F", "2"= "M"))

#Mutating to apply new names
total5<-mutate(total3,Gender=total4)

#Changing diagnosis
total6<-str_replace_all(total5$Diagnosis, c("A" = "ASD", "B"= "TD"))

#Mutating to apply new names
cleandata<-mutate(total5,Diagnosis=total6)
